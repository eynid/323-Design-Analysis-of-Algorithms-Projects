/*
*************************
Purpose of this project is to demonstrate the idea of a 2-3 tree and 
how it would work. 
Input is p4data.txt and p4data2.txt
...
class treeNode{
    ...
    //Printing out nodes, if a node does not have a parent, we have to account for such measures and print accordingly. 
...
class Tree{
    ...
    treeNode findSpot(int data, treeNode Spot, FileWriter debug) throws IOException{
        ...
        if(isLeaf(Spot.child1)){
            ...
            //There are 3 cases to consider when inserting into a 2-3 tree. The first is the easiest, if the spot is 
            //empty and the data is less than the first key, it will go to the first child. 
            if(data < Spot.key1){
                return findSpot(data, Spot.child1, debug);
            
            }
            //The second case to consider is if the second key for the parent does not exist, or if data is less than the parent's key2 but,
            // gteater than key1, we insert the data into the second child. 
